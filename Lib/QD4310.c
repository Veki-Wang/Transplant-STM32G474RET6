...
static uint8_t RxBuffer[10];//原来是static uint8_t RBuffer[10];
...
void QD4310_SendCommand(QD4310_t *motor, QD4310_Command_t cmd, int16_t value) {
    ...
    HAL_UART_Transmit(motor->huart, TxBuffer, sizeof(TxBuffer), HAL_MAX_DELAY);
    // 串口无法做到绝对0丢包，必须设置Timeout
    // 提高波特率后可以适当减小Timeout
    HAL_UART_Receive(motor->huart, RxBuffer, sizeof(RxBuffer), 5);
    if (RxBuffer[0] == motor->id && CRC8(RxBuffer, 9, 0x07, 0x00, 0x00, false, false) == RxBuffer[9]) {
        QD4310_Update(motor, RxBuffer + 1);
    }
}
...
void QD4310_Reboot(QD4310_t *motor) {
    QD4310_SendCommand(motor, QD4310_CMD_REBOOT, 0x0000);
}

// 设置电机零点
void QD4310_SetZeroPos(QD4310_t *motor) {
    QD4310_SendCommand(motor, QD4310_CMD_SET_ZERO_POS, 0x0000);
}
